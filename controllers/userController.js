...
exports.getGardenerProFile = async (req, res, next) => {
  try {
    const gardener = await User.findAll({
      where: {
        [Op.and]: [{ role: "gardener" }, { status: "active" }],
      },
    });
...
exports.getAllProFileUser = async (req, res, next) => {
  try {
    ...
    if (status === "all") {
      ...
      profileAll = await User.findAll({
        where: {
          [Op.and]: [{ role: ["sales", "gardener"] }, { status }],
        },
        attributes: [
          "id",
          "firstName",
...
exports.getSearchProFileUser = async (req, res, next) => {
  try {
    ...
    if (search === "admin")
      return res.status(400).json({ message: " Can't find this role." });
  ...
  try {
    ...
    if (
      ...
      return res.status(400).json({
        message:
          "Password must contain at least eight but not more than fifteen characters, Which contain at least one uppercase letters, lowercase letters, numbers, and special characters.",
      });
    }
    ...
    if (nationalId.length !== 13)
      return res
        .status(400)
        .json({ message: "National id must have 13 digits." });
    const digit = checkNationalID(nationalId);
    if (nationalId.slice(-1) !== digit)
      return res.status(400).json({ message: "National id is Invalid." });
    let approved;
    if (role !== "admin") {
...
exports.changePassword = async (req, res, next) => {
  ...
  try {
    ...
    if (newConfirmPassword === undefined)
      return res
        .status(400)
        .json({ message: "New confirm password is required" });
    if (newPassword !== newConfirmPassword)
      return res.status(400).json({ message: "New password did not match" });
    const user = await User.findOne(
      {
        where: { id: req.user.id },
      },
