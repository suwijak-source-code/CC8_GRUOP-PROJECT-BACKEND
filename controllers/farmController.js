...
exports.getAllFarm = async (req, res, next) => {
  try {
    const farms = await Farm.findAll({
      include: [
        ...
        {
          model: Planting,
          include: [PlantingTransaction, Seed],
          //   order: [[Planting, "id", "DESC"]],
        },
      ],
      order: [
        ["id", "asc"],
...
exports.createFarm = async (req, res, next) => {
  ...
  try {
    ...
    if (farmApprovedFName === undefined)
      return res
        .status(400)
        .json({ message: "First name of approver is required." });
    if (farmApprovedLName === undefined)
      return res
        .status(400)
...
exports.editFarm = async (req, res, next) => {
  ...
  try {
    const { id } = req.params;
    let { farmName, status, farmRemark, farmApprovedFName, farmApprovedLName } =
      req.body;
    if (farmName === undefined) {
