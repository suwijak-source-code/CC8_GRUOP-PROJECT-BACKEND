const { User, sequelize, Farm, Seed, Customer, Planting } = require('../models');
...
cron.schedule('0 0 * * *', async function () {
    try {
        ...
        harvested.map(async (item) => await Planting.update({
            status: 'harvested', assignStatus: 'notAssign'
        }, { where: { id: item.id } })
        );
...
exports.createPlanting = async (req, res, next) => {
    ...
    try {
        const { farmName, seedName, startDate, status, customerName, plantingRemark, plantedAmount,
            plantingApprovedFName, plantingApprovedLName, fertilizerPrice, maintenanceCost,
            miscellaneousExpenses } = req.body;
        if (fertilizerPrice === undefined) return res.status(400).json({ message: 'Fertilizer price is required.' });
        if (isNaN(+fertilizerPrice) || fertilizerPrice === null) return res.status(400).json({ message: 'Fertilizer price must be a number.' });
        if (maintenanceCost === undefined) return res.status(400).json({ message: 'Maintenance cost is required.' });
        if (isNaN(+maintenanceCost) || maintenanceCost === null) return res.status(400).json({ message: 'Maintenance cost must be a number.' });
        if (miscellaneousExpenses === undefined) return res.status(400).json({ message: 'Miscellaneous expenses is required.' });
        if (isNaN(+miscellaneousExpenses) || miscellaneousExpenses === null) return res.status(400).json({ message: 'Miscellaneous expenses must be a number.' });
        ...
        if (plantingApprovedFName === undefined) return res.status(400).json({ message: 'First name of approver is required.' });
        if (plantingApprovedLName === undefined) return res.status(400).json({ message: 'Last name of approver is required.' });
        ...
        if (!plantingApproved) return res.status(400).json({ message: 'Approve name not found.' });
        if (status === undefined) return res.status(400).json({ message: 'Status date is required.' });
        if (status !== 'started' && status !== 'harvested' && status !== 'finished' && status !== 'cancel')
            return res.status(400).json({ message: 'Status not found.' });
        let date = moment(startDate, "YYYY/MM/DD");
        const harvestDate = moment(date).add(seed.plantingTime, "days");
        if (plantedAmount === undefined) return res.status(400).json({ message: 'Planted amount is required.' });
        if (isNaN(+plantedAmount) || plantedAmount === null)
            return res.status(400).json({ message: 'Planted amount must be a number.' });
        const planting = await Planting.create({
            farmId: farm.id, seedId: seed.id, startDate, status, reservedCustomer: customer, remark: plantingRemark,
            plantingApprovedBy: plantingApproved.id, harvestDate, plantedAmount, assignStatus: 'notAssign', fertilizerPrice,
            maintenanceCost, miscellaneousExpenses
        }, { transaction: trans });
        await trans.commit();
...
exports.editPlanting = async (req, res, next) => {
    ...
    try {
        const { id } = req.params;
        let { farmName, seedName, startDate, customerName, plantingRemark, plantedAmount,
            plantingApprovedFName, plantingApprovedLName, fertilizerPrice, maintenanceCost,
            miscellaneousExpenses } = req.body;
        ...
        if (isNaN(+fertilizerPrice) || fertilizerPrice === null)
            return res.status(400).json({ message: 'Fertilizer price must be a number.' });
        ...
        if (isNaN(+maintenanceCost) || maintenanceCost === null)
            return res.status(400).json({ message: 'Maintenance cost must be a number.' });
        ...
        if (isNaN(+miscellaneousExpenses) || miscellaneousExpenses === null)
            return res.status(400).json({ message: 'Miscellaneous expenses must be a number.' });

        await Planting.update({
            farmId: farm, seedId: seed, startDate, status, reservedCustomer: customer, remark: plantingRemark,
            plantingApprovedBy: plantingApproved.id, harvestDate, plantedAmount, fertilizerPrice, maintenanceCost,
    ...
    try {
        ...
        if (plantingTime) {
            ...
            startDatePlanting.map(async (item) => {
                ...
                await Planting.update({
                    harvestDate
                }, { where: { seedId: id } }, { transaction: trans })
            });
            await trans.commit();
            ...
            if (cancelJob.mission === 'gardening') {
                ...
                const newStartDate = moment(datePlanting.startDate, "YYYY/MM/DD").add(1, "days");
                const newHarvestDate = moment(datePlanting.harvestDate, "YYYY/MM/DD").add(1, "days");
                await Planting.update({
                    startDate: newStartDate, harvestDate: newHarvestDate, assignStatus: 'notAssign'
                }, { where: { id: cancelJob.plantingId } }, { transaction: trans });
                await trans.commit();
                ...
                const newHarvestDate = moment(harvestDatePlanting.harvestDate, "YYYY/MM/DD").add(1, "days");
                await Planting.update({
                    harvestDate: newHarvestDate, assignStatus: 'notAssign', status: 'started'
                }, { where: { id: cancelJob.plantingId } }, { transaction: trans });
                await trans.commit();
                res.status(200).json({ message: 'Cancel job successfully' });
            };
            ...
            if (cancelJob.status === 'started') {
                const newStartDate = moment(cancelJob.startDate, "YYYY/MM/DD").add(1, "days");
                const newHarvestDate = moment(cancelJob.harvestDate, "YYYY/MM/DD").add(1, "days");
                await Planting.update({
                    startDate: newStartDate, harvestDate: newHarvestDate
                }, { where: { id } }, { transaction: trans });
                await trans.commit();
                res.status(200).json({ message: 'Cancel job successfully' });
...
exports.updateHarvestedAmount = async (req, res, next) => {
    ...
    try {
        ...
        if (updateStatusPlanting) {
            if (harvestedAmount === undefined) return res.status(400).json({ message: 'Harvested amount is required.' });
            if (isNaN(+harvestedAmount) || harvestedAmount === null)
                return res.status(400).json({ message: 'Harvested amount must be a number.' });
            await Planting.update({
                harvestedAmount
            }, { where: { id: updateStatusPlanting.plantingId } }, { transaction: trans });
            await trans.commit();
            next();
        } else {
            const { id } = req.params;
...
exports.updateStatus = async (req, res, next) => {
    ...
    try {
        ...
        if (updateStatusPlanting) {
            ...
            if (status === 'cancel') {
                await Planting.update({
                    status, canceler: req.user.id
                }, { where: { id } }, { transaction: trans });
                const plantingCancel = await Planting.findOne({ where: { id } }, { transaction: trans })
                await trans.commit();
...
exports.updateAssignStatus = async (req, res, next) => {
    ...
    try {
        ...
        await Planting.update({
            assignStatus
        }, { where: { id: plantingId } }, { transaction: trans });
        await trans.commit();
        next();
    } catch (err) {
        await trans.rollback();
